        ...
        for deck_num, cards in DECK_GRAPHICS.items():
            ...
            for card_num, elements in cards.items():
                ...
                for elem in elements:
                    ...
                    if elem["type"] == "progress_bar" and deck_num == 14 and card_num == 1:
                        pb_label = QLabel("Progress Bar Value:")
                        pb_input = QLineEdit("212")
                        pb_input.setFixedWidth(80)
                        pb_input.setPlaceholderText("0-250")
                        btn_set_pb = QPushButton("Set Progress Bar")
                        btn_set_pb.setFixedWidth(120)
                        btn_set_pb.clicked.connect(lambda _, t=pb_input: self.queue.send_progress_bar(14, 1, t.text()))
                        row.addWidget(pb_label)
                        row.addWidget(pb_input)
                        row.addWidget(btn_set_pb)
