...
class TellTaleWidget(QWidget):
    ...
    def init_ui(self):
        for elem in self.elements:
            if elem["type"] == "image":
                ...
                if duration > 0:  # 0 means infinite
    ...
    def setup_chime(self, chime_name, chime_type):
        ...
        if chime_file:
            ...
            if os.path.exists(chime_path):
                ...
                if chime_type == "once":
                    # Use QTimer to ensure media is loaded before playing
                    QTimer.singleShot(200, self.play_chime)
                    logger.info(f"Chime activated (once) - Deck: {self.deck_num}, Card: {self.card_num}, Chime: {chime_name}")
                elif chime_type == "twice":
                    # Use QTimer to ensure media is loaded before playing
                    QTimer.singleShot(200, self.play_chime)
                    logger.info(f"Chime activated (twice) - Deck: {self.deck_num}, Card: {self.card_num}, Chime: {chime_name}")
                elif chime_type == "continuous":
                    # Create timer for continuous chime
                    ...
                    # Use QTimer to ensure media is loaded before playing
                    QTimer.singleShot(200, self.play_chime)
                    logger.info(f"Chime activated (continuous) - Deck: {self.deck_num}, Card: {self.card_num}, Chime: {chime_name}")
            else:
    ...
    def on_chime_status_changed(self, status):
        ...
        if status == QMediaPlayer.EndOfMedia and self.chime_type == "twice" and self.chime_play_count < 2:
            ...
            if self.chime_play_count < 2:
                ...
                QTimer.singleShot(800, self.play_chime)
    ...
    def play_chime(self):
        ...
        if self.chime_player:
            status = self.chime_player.mediaStatus()
            if status == QMediaPlayer.LoadedMedia or status == QMediaPlayer.BufferedMedia:
                self.chime_player.play()
                logger.debug(f"Chime played - Deck: {self.deck_num}, Card: {self.card_num}")
            else:
                # If not loaded, try again after a short delay
                QTimer.singleShot(300, self.play_chime)
                logger.debug(f"Chime not ready, retrying - Deck: {self.deck_num}, Card: {self.card_num}, Status: {status}")
